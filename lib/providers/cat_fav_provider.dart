...
class CatFavProvider {
  ...
  int _limit = 10; // * CAMBIAR LUEGO XD
  ...
  Future<List<Cat>> getFavoriteCats() async {
    ...
    final url = Uri.https(_url, '/v1/favourites', {'sub_id': _sub_id, 'limit': _limit.toString(), 'order':'DESC'});
    ...
    final allCatsFutures = catsFav.items.map((catFav) => getCatInfo(catFav)).toList();
  ...
  void addToFavorites(Cat cat) async {
    ...
    final decodedData = json.decode(resp.body);
    final catFav = CatFav.fromJsonMap(decodedData);
